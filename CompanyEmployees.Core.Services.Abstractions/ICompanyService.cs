...
public interface ICompanyService
{
    ...
    Task<(IEnumerable<CompanyDto> companies, string ids)> CreateCompanyCollectionAsync(IEnumerable<CompanyForCreationDto> companyCollection, CancellationToken ct = default);
    Task DeleteCompanyAsync(Guid companyId, bool trackChanges, CancellationToken ct = default);
    Task UpdateCompanyAsync(Guid companyid, CompanyForUpdateDto companyForUpdate, bool trackChanges, CancellationToken ct = default);
}